...
### 禁止用 `Command::new("where")` / `which` 类子进程查找可执行文件

**背景**：桌面端 `find_claude_exe()` 曾用 `Command::new("where").arg("claude").output()` 启动 `where.exe` 子进程查找 claude 路径。release 版 `cc-start.exe` 调用这个子进程会**稳定卡约 5 秒返回**（同一份代码在 dev 模式下却只要 < 10ms，底层差异未定位，可能与 WebView2 注入 hook、PATH 中某条慢条目相关）。结果就是点【启动】按钮要等 5-6 秒 wt 窗口才弹出来。

**修复**（2026-05-20，分支 `bugfix/desktop-slow-launch`）：`find_claude_exe()` 改为纯 Rust 内部扫 `PATH` + `PATHEXT`，不再 spawn 任何子进程。实测 `find_claude_exe()` 从 5170ms 降到 7-8ms，总 invoke 从 5200ms 降到 22-38ms。

**约束**：今后桌面端 Rust 代码**禁止**用 `Command::new(...)` spawn `where` / `which` / `cmd /c where ...` 等系统工具子进程做"查 PATH"或"找可执行文件"。能用 Rust 标准库在进程内解决的，绝不 spawn 子进程——`cc-start.exe` 这个父进程上下文里，看似无害的子进程调用可能莫名其妙慢几个数量级。

**调试约束**：桌面端启动性能类问题**必须**用 release build（`cd desktop && npm run tauri build -- --no-bundle`）验证；dev 模式（`npm run tauri dev`）行为不同，可能完全掩盖问题。这次 bug 就是因为长期只在 dev 模式自测，根因被掩盖到了 release 才暴露。
